<template>
  <span class="search_title">考试搜索</span>
  <el-form class="searchForm">
    <el-form-item label="考试代码">
      <el-input style="width: 83%" v-model="searchForm.exam_name" placeholder="请输入试卷代码"/>
    </el-form-item>
    <el-form-item label="考试名称">
      <el-input style="width: 83%" v-model="searchForm.exam_id" placeholder="请输入试卷名称"/>
    </el-form-item>
    <el-form-item label="所属课程">
      <el-select style="width: 83%" v-model="searchForm.course_id" placeholder="请选择所属课程">
        <el-option
            v-for="item in option.course"
            :label="item.course_name"
            :value="item.course_id" />
      </el-select>
    </el-form-item>
    <el-form-item label="考试类型">
      <el-select style="width: 83%" v-model="searchForm.exam_type" placeholder="请选择考试类型">
        <el-option
            v-for="item in exam_type"
            :label="item.label"
            :value="item.label" />
      </el-select>
    </el-form-item>
    <el-form-item class="formFooter">
      <el-button @click="reset()">重置</el-button>
      <el-button @click="search()" type="primary">搜索</el-button>
    </el-form-item>
  </el-form>

</template>

<script setup>
import {onMounted, reactive, ref} from "vue";
import https from "@/apis/axio";
import {ElMessage} from "element-plus"

const emit = defineEmits(['search_exam'])

const option = reactive({
  course:[]
})
const exam_type=[{'label':'随堂练习'},{'label':'期中考试'},{'label':'期末考试'}]
const searchForm = reactive({
  exam_name: '',
  exam_id:'',
  exam_course:'',
  exam_type:''
})

onMounted(()=>{
  https.post('/teacher/get_courses',{'username':JSON.parse(localStorage.getItem('token')).username}).then(res => {
    option.course = res.data
  })
})

function search() {
  // https.post('/teacher/search_paper',searchForm).then(res => {
  //   res.data.forEach(item=>{
  //     item.catalog_name=item.catalog_name.join('，')
  //   })
  //   emit('search_exam',res.data)
  // })
}

function reset() {
  searchForm.exam_name=''
  searchForm.exam_id=''
  searchForm.exam_course=''
  searchForm.exam_type=''
}

</script>

<style scoped lang="scss">
.search_title{
  display: block;
  font-size: 22px;
  font-weight: bold;
  padding-left: 10px;
  padding-top: 15px;
}
.searchForm{
  margin-top: 20px;
  margin-left: 10px;
}
.el-form-item{
  font-weight: bold;
  font-size: 5px;
}

.formFooter{
  float: right;
}

</style>
