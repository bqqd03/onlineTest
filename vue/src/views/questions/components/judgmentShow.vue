<template>
  <el-card v-for="(item) in props.judgments" :key="item.id" style="margin-bottom: 12px">
    <div class="card-header">
      <div>
        <span style="color: #409eff;">【判断题】</span>
    ...
    <div style="margin-left: 10px;display: flex;flex-flow: column;" >
      <el-radio label="T" :model-value=item.answer />
      <el-radio label="F" :model-value=item.answer />
...
function deleteQuestion(e) {
  https.post('/teacher/delete_question', {'question_id':e.id,'question_type':'判断题'}).then(res=>{
